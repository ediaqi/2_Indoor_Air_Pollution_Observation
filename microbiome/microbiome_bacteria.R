...
ps_dust = subset_taxa(ps_dust, Kingdom=="Bacteria"); ps_dust
#ps_dust = subset_taxa(ps_dust, Order!="Chloroplast"); ps_dust 
#ps_dust = subset_taxa(ps_dust, Family!="Mitochondria"); ps_dust
...
############ Decontam if necessary/possible ##############

sample_data(ps_dust)$is.neg <- sample_data(ps_dust)$Sample_or_Control == "Control"

contamdf.prev_dust <- isContaminant(ps_dust, method="prevalence", neg="is.neg")
table(contamdf.prev_dust$contaminant)
head(which(contamdf.prev_dust$contaminant))

contamdf.prev05_dust <- isContaminant(dust, method="prevalence", neg="is.neg", threshold=0.5)
table(contamdf.prev05_dust$contaminant)
head(which(contamdf.prev05_dust$contaminant))


ps.pa_dust <- transform_sample_counts(ps_dust, function(abund) 1*(abund>0))
ps.pa.neg_dust <- prune_samples(sample_data(ps.pa_dust)$Sample_or_Control == "Control", ps.pa_dust)
ps.pa.pos_dust <- prune_samples(sample_data(ps.pa_dust)$Sample_or_Control == "Sample", ps.pa_dust)
df.pa_dust <- data.frame(pa.pos_dust=taxa_sums(ps.pa.pos_dust), pa.neg_dust=taxa_sums(ps.pa.neg_dust),
                          contaminant=contamdf.prev_dust$contaminant)
ggplot(data=df.pa_dust, aes(x=pa.neg_dust, y=pa.pos_dust, color=contaminant)) + geom_point() +
  xlab("Prevalence (Negative Controls)") + ylab("Prevalence (True Samples)")

ps_dust_decontam_05 <- prune_taxa(!contamdf.prev05_dust$contaminant, ps_dust)
ps_dust_decontam_05

ps_dust_decontam_01 <- prune_taxa(!contamdf.prev_dust$contaminant, ps_dust)
ps_dust_decontam_01
...
dispersion_BAC_dust<- vegan::betadisper(phyloseq_bray_CSS_dust, sampledf_CSS_dust$Health_status, bias.adjust = TRUE, sqrt.dist = TRUE)
anova(dispersion_BAC_dust) #
vegan::permutest(dispersion_BAC_dust, pairwise=TRUE, permutations=999)
plot(dispersion_BAC_dust)
boxplot(dispersion_BAC_dust, main = "", xlab = "") #visulaize the data dispersion


############ Alpha diversity anaylsis ######
...
subsamp_dust_min <- rarefy_even_depth(ps_dust, sample.size=min(ps_dust), rngseed=5163, replace=FALSE, trimOTUs=TRUE); subsamp_dust_min #11370  a 689
ggrare(subsamp_dust_min, step= 50, color="Health_status", se=FALSE) 

Bacterial_richness_dust_min = estimate_richness(subsamp_dust_min , measures=c("Observed","Shannon", "Chao1", "InvSimpson"))
...
Bacterial_richness_dust_min$SampleID<- rownames(Bacterial_richness_dust_min)
Bacterial_richness_dust_min$SampleID<-rownames(meta_ps_dust_min)
Bacterial_richness_dust_min<-full_join(Bacterial_richness_dust_min, meta_ps_dust_min, by="SampleID");Bacterial_richness_dust_min


shapiro.test(Bacterial_richness_dust_min$Shannon) #
# -> data is normally distributed!!!
hist(Bacterial_richness_dust_min$Shannon, col='steelblue')
qqnorm(Bacterial_richness_dust_min$Shannon)
qqline(Bacterial_richness_dust_min$Shannon)

one.way.Shannon <- aov(Shannon ~ identity, data = Bacterial_richness_dust_min)
kruskal.test(Shannon ~ harvest_year, data = Bacterial_richness_dust_min)

Shannon_all_Healthy_PW <- pairwise.wilcox.test(Bacterial_richness_dust_min$Shannon, Bacterial_richness_dust_min$Health_status,p.adjust.method="fdr") #for exact numbers
...
shannon_country_field<-ggboxplot(Bacterial_richness_dust_min, x = "Health_status", y = "Shannon", fill= "Health_status", ylab = "Shannon Index", xlab="", add = "dotplot", legend="") +  
...
shapiro.test(Bacterial_richness_dust_min$Observed) #
# -> data is normally distributed!!!
hist(Bacterial_richness_dust_min$Observed, col='steelblue')
qqnorm(Bacterial_richness_dust_min$Observed)
qqline(Bacterial_richness_dust_min$Observed)

one.way.Observed <- aov(Observed ~ identity, data = Bacterial_richness_country)
kruskal.test(Observed ~ harvest_year, data = Bacterial_richness_country)

Observed_all_Healthy_PW <- pairwise.wilcox.test(Bacterial_richness_dust_min$Observed, Bacterial_richness_dust_min$Health_status,p.adjust.method="fdr") #for exact numbers
...
Observed_country_field<-ggboxplot(Bacterial_richness_dust_min, x = "Health_status", y = "Observed", fill= "Health_status", ylab = "Number of ASVs", xlab="", add = "dotplot", legend="") +  
  #theme(axis.text.x=element_text(angle=90)) + 
  theme() + 
  #facet_wrap(~Health_status, scales="free", labeller=as_labeller(plot_names), nrow=1)+  
  scale_x_discrete(labels=c("Healthy", "Sick"))#+
...
reduced.model <- glm(Shannon ~ x, data = alpha_dust_par_transf) #x represents the significant factors from anova
